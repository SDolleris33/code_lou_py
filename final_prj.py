...
while True:
	...
	if nav not in NAV_CONT:
		...
		if len(cart_items) == 0:
			...
			print(f"""# of items: {len(cart_items)}\n Titles: {str(game_names(cart_items))}\n Total: ${sum(cart_prices)}""")
		...
		if username == "Admin" and password == "password":
			game = []
			game.append(int(len(game_inv)+1))
			game.append(input("Enter game Title: "))
			game.append(int(input("Enter price !!! dont not include '$' !!!: ")))	
